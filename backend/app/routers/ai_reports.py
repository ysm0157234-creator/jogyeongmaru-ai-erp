...
def get_owned_draft(
    ...
    if not draft:
        raise HTTPException(
            status_code=404,
            detail="AI 신고 초안을 찾을 수 없습니다.",
        )
    if draft.created_by != user.id:
        raise HTTPException(
            status_code=403,
            detail="이 신고 초안에 접근할 권한이 없습니다.",
        )
    return draft


def validate_result_data(result_data: dict) -> None:
    selected = result_data.get("selected_images", {})
    overall = selected.get("overall")
    closeup = selected.get("closeup")

    if not overall or not closeup:
        raise HTTPException(
            status_code=422,
    ...
    if not str(result_data.get("characteristics_draft", "")).strip():
        raise HTTPException(
            status_code=422,
            detail="품종 특성 설명이 비어 있습니다.",
        )
    if not str(result_data.get("breeding_process_draft", "")).strip():
        raise HTTPException(
            status_code=422,
            detail="품종 육성과정이 비어 있습니다.",
        )
...
def drive_status(
    ...
    checks = {
        ...
        "SHIPMENT_OVERVIEW_FILE_ID": bool(
            settings.shipment_overview_file_id.strip()
        ),
        "IMPORT_2025_FOLDER_ID": bool(
            settings.import_2025_folder_id.strip()
        ),
        "GOOGLE_SEARCH_API_KEY": bool(
            settings.google_search_api_key.strip()
        ),
        "GOOGLE_SEARCH_ENGINE_ID": bool(
            settings.google_search_engine_id.strip()
        ),
        "GEMINI_API_KEY": bool(
    ...
    return {
        ...
        "message": (
            "Google Drive·검색·Gemini 연결 준비 완료"
            if not missing
...
def generate_ai_report(
    ...
    if not variety_name:
        raise HTTPException(
            status_code=422,
            detail="신고할 품종명을 입력하세요.",
        )

    try:
        result = research_variety(
            variety_name,
            payload.agency,
        )
    except (
    ...
    except (GoogleSearchError, PlantResearchError) as error:
        raise HTTPException(
            status_code=502,
            detail=str(error),
        ) from error
        ...
        raise HTTPException(
            status_code=500,
            detail=f"품종 인터넷 조사 중 오류: {type(error).__name__}: {error}",
        ) from error
...
@router.get(
    "",
    response_model=list[AIDraftResponse],
)
def list_ai_drafts(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
    ...
    return (
        db.query(AIDraft)
        .filter(AIDraft.created_by == current_user.id)
...
def generate_files(
    ...
    try:
        zip_bytes, manifest = run_workflow(
            variety_name,
            copy.deepcopy(draft.result_data),
        )
        if not zip_bytes:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"Google Drive 처리 오류: {error}",
        ) from error
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": (
                'attachment; filename="production_sales_report_complete.zip"'
            ),
